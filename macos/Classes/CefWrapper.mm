...
+ (BOOL)processKeyboardEvent: (NSEvent*) event {
    NSView* eventView = event.window.contentView;
    if (![eventView isKindOfClass:NSClassFromString(@"FlutterViewWrapper")]) {
        eventView = eventView.subviews.firstObject;
    }
    CefWrapper *currentPlugin = [webviewPlugins objectForKey:eventView];
...
- (id) init {
    ...
    if (self) {
        ...
        if(isCefProcessInit == NO){
            webview_cef::initCEFProcesses(0, NULL);
            isCefProcessInit = YES;
        }
        _plugin->setInvokeMethodFunc([=](const std::string& method, WValue* arguments){
