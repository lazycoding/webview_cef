...
bool CefJSBridge::ExecuteJSCallbackFunc(int callbackId, bool error, const CefString& result, bool deleteCallBackFunction)
{
	if (callbackId < 0)
	{
		...
		if (it != startRequest_callback_.cend())
		{
			...
			{
				std::ostringstream strStream;
				strStream <<"window['" << callback.ToString() << "'](" << callbackId * -1 << ", " << result.ToString() << ", " << deleteCallBackFunction << ");";
				strStream.flush();
