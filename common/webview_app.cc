...
void WebviewApp::OnBeforeCommandLineProcessing(
    ...
    if (process_type.empty()) {
        ...
        command_line->AppendSwitch("allow-running-insecure-content");  // allow running insecure
                                                                       // content in secure pages
        ...
        if (m_uMode == 1) {
            ...
            command_line->AppendSwitchWithValue(
                    ...
                    "8");  // limit renderer process count to decrease memory usage
        } else if (m_uMode == 2) {
        ...
        command_line->AppendSwitchWithValue(
                "autoplay-policy", "no-user-gesture-required");  // autoplay policy for media
        ...
        if (!m_strFilterDomain.empty()) {
            ...
            command_line->AppendSwitchWithValue(
                    "unsafely-treat-insecure-origin-as-secure", m_strFilterDomain);
        }
    }
...
void WebviewApp::OnContextInitialized() {
    ...
    //
    //    // create browser
    //    CefBrowserHost::CreateBrowser(window_info, m_webviewHandler, "", browser_settings,
    //    nullptr, nullptr);
}
    ...
    std::string extensionCode = R"(
			...
			(() => {
                ...
                external.JavaScriptChannel = (n, e, r) => {
                    var a = '_' + new Date + (1e3 + Math.floor(8999 * Math.random()))
                    if(n != 'BDHScript.ThrowEvent'){
                        window[a] = function (n, a, e) {
                            return function () {
                                var d = true
                                try {
                                    var resParams = arguments[1];
                                    if (arguments.length >= 3) {
                                        d = arguments[2];
                                    }
                                    switch (resParams.code) {
                                    case 200:
                                        e.onsuccess && e.onsuccess(resParams.data)
                                        break;
                                    case 202:
                                        e.oncancel && e.oncancel(resParams.data)
                                        break;
                                    case 203:
                                        e.fail && e.fail(resParams.data)
                                        e.onfail && e.onfail(resParams.data)
                                        break;
                                    case 210:
                                        e.listener && e.listener(resParams.data)
                                        break;
                                    case 301:
                                        e.onconnect && e.onconnect(resParams.data)
                                        break;
                                    case 302:
                                        e.ondisconnect && e.ondisconnect(resParams.data)
                                        break;
                                    case 303:
                                        e.onreceiveMsg && e.onreceiveMsg(resParams.data)
                                        break;
                                    case 304:
                                        e.onchange && e.onchange(resParams.data)
                                        break;
                                    case 310:
                                        e.onReady && e.onReady(resParams.data)
                                        break;
                                    case 311:
                                        e.onProgress && e.onProgress(resParams.data)
                                        break;
                                    case 312:
                                        e.onComplete && e.onComplete(resParams.data)
                                        if (isTask && window.BdHiJs) {
                                            typeof isTask === 'function' ? isTask(resParams.data) : window.BdHiJs.appnative.webview.taskCallbackConfirm({
                                                data: { taskID: resParams.data.taskID }
                                            })
                                        }
                                        break;
                                    }
                                    ...
                                    d = true
                                    console.log('callback catch' + l)
                                } finally {
                                    if (d) {
                                        delete window[a]
                                        console.log(n + ' callback done, delete handler with id ' + a)
                                    }
                                }
                            }
                        }(n, a, e);
                        ...
                        null == r ? a = '' : (a = '_' + new Date + (1e3 + Math.floor(8999 * Math.random())), window[a] = function (n, e) { 
                            return function () {
                                ...
                                try {
                                    ...
                                    window[e] && window[e].call && window[e].call(null, arguments[1]);
                                } finally {
                                    if (d) {
                                        delete window[n]
                                    }
                        ...
                        try {
                            external.StartRequest(external.GetNextReqID(), 'BDHScript.ThrowEvent', a, JSON.stringify({'method':e,'param':r} || {}), '')
                        } catch (l) {
                            console.log('messeage send')
                        }
                    }
                ...
                BDHScript.ThrowEvent =(e,r) => {external.JavaScriptChannel("BDHScript.ThrowEvent", e, r)};
...
void WebviewApp::OnBrowserCreated(
...
void WebviewApp::OnContextCreated(
        CefRefPtr<CefBrowser> browser,
        CefRefPtr<CefFrame> frame,
        CefRefPtr<CefV8Context> context) {}

void WebviewApp::OnContextReleased(
        CefRefPtr<CefBrowser> browser,
        CefRefPtr<CefFrame> frame,
        CefRefPtr<CefV8Context> context) {
    if (m_render_js_bridge.get()) {
...
void WebviewApp::OnUncaughtException(
        CefRefPtr<CefBrowser> browser,
        CefRefPtr<CefFrame> frame,
        CefRefPtr<CefV8Context> context,
...
void WebviewApp::OnFocusedNodeChanged(
        CefRefPtr<CefBrowser> browser,
        CefRefPtr<CefFrame> frame,
    ...
    if (is_editable) {
        ...
        message->GetArgumentList()->SetInt(2, rect.y);
        message->GetArgumentList()->SetInt(3, rect.width);
        message->GetArgumentList()->SetInt(4, rect.height);
    }
...
bool WebviewApp::OnProcessMessageReceived(
    ...
    if (message_name == kExecuteJsCallbackMessage) {
        ...
        bool deleteCallBackFunction = message->GetArgumentList()->GetBool(3);
        if (m_render_js_bridge.get()) {
            m_render_js_bridge->ExecuteJSCallbackFunc(
                    callbackId, error, result, deleteCallBackFunction);
        }
    }
