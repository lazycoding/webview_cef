...
public:
    WebviewTextureRenderer(FlutterDesktopTextureRegistrarRef texture_registrar) {
        ...
        info.pixel_buffer_config.callback = [](size_t width, size_t height, void* user_data)
    ...
    virtual ~WebviewTextureRenderer() {
        ...
        if (registrar_) {
            // FlutterDesktopTextureRegistrarUnregisterExternalTexture(registrar_, textureId,
            // nullptr, nullptr);
        }
    }
    ...
    virtual void onFrame(const void* buffer, int width, int height) override {
        ...
        if (pixel_buffer && (pixel_buffer->width != width || pixel_buffer->height != height)) {
...
static flutter::EncodableValue encode_wvalue_to_flvalue(WValue* args) {
    ...
    switch (type) {
        ...
        case Webview_Value_Type_Uint8_List: {
            ...
            auto values = webview_value_get_uint8_list(args);
            return flutter::EncodableValue(std::vector<uint8_t>(values, values + len));
        }
        case Webview_Value_Type_Int32_List: {
            size_t len = webview_value_get_len(args);
            auto values = webview_value_get_int32_list(args);
            return flutter::EncodableValue(std::vector<int32_t>(values, values + len));
        }
        case Webview_Value_Type_Int64_List: {
            size_t len = webview_value_get_len(args);
            auto values = webview_value_get_int64_list(args);
            return flutter::EncodableValue(std::vector<int64_t>(values, values + len));
        }
        case Webview_Value_Type_Float_List: {
            size_t len = webview_value_get_len(args);
            auto values = webview_value_get_float_list(args);
            return flutter::EncodableValue(std::vector<float>(values, values + len));
        }
        case Webview_Value_Type_Double_List: {
            size_t len = webview_value_get_len(args);
            auto values = webview_value_get_double_list(args);
            return flutter::EncodableValue(std::vector<double>(values, values + len));
        }
        ...
        case Webview_Value_Type_Map: {
            ...
            size_t len = webview_value_get_len(args);
            for (size_t i = 0; i < len; i++) {
                ret[encode_wvalue_to_flvalue(webview_value_get_key(args, i))] =
                        encode_wvalue_to_flvalue(webview_value_get_value(args, i));
            }
            return ret;
        }
        default:
...
static WValue* encode_flvalue_to_wvalue(flutter::EncodableValue* args) {
    ...
    if (index == 1) {
        ...
        for (flutter::EncodableMap::iterator it = map.begin(); it != map.end(); it++) {
            WValue* key =
                    encode_flvalue_to_wvalue(const_cast<flutter::EncodableValue*>(&it->first));
            WValue* value =
...
std::unordered_map<
        HWND,
        std::function<void(std::string method, flutter::EncodableValue* arguments)>>
        webviewChannels;
...
void WebviewCefPlugin::RegisterWithRegistrar(FlutterDesktopPluginRegistrarRef registrar) {
    ...
    plugin->m_channel = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
    ...
    webviewChannels.emplace(
            plugin->m_hWnd,
            [plugin_pointer = plugin.get()](
                    std::string method, flutter::EncodableValue* arguments) {
                plugin_pointer->m_channel->InvokeMethod(
                        method, std::make_unique<flutter::EncodableValue>(*arguments));
            });
    plugin->m_plugin->setInvokeMethodFunc(
            [plugin_pointer = plugin.get()](const std::string& method, WValue* arguments) {
                flutter::EncodableValue* methodValue = new flutter::EncodableValue(method);
                flutter::EncodableValue* args =
                        new flutter::EncodableValue(encode_wvalue_to_flvalue(arguments));
    ...
    plugin->m_plugin->setCreateTextureFunc([plugin_pointer = plugin.get()]() {
        std::shared_ptr<WebviewTexture> renderer =
                std::make_shared<WebviewTextureRenderer>(plugin_pointer->m_textureRegistrar);
        return renderer;
    });
...
WebviewCefPlugin::WebviewCefPlugin() :
        m_hWnd(NULL), m_mainThreadId(0), m_textureRegistrar(nullptr) {
...
void WebviewCefPlugin::HandleMethodCall(
    ...
    WValue* encodeArgs =
            encode_flvalue_to_wvalue(const_cast<flutter::EncodableValue*>(method_call.arguments()));
